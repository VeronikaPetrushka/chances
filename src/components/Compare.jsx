...
const Compare = () => {
    ...
    const loadCompareSelectedOptions = async () => {
        try {
            const stored = await AsyncStorage.getItem('compareSelectedOptions');
            if (stored) {
    ...
    console.log('compareSelectedOptions: ', compareSelectedOptions)
    ...
    const toggleSelectedCompareOptions = async (option) => {
        try {
            ...
            await AsyncStorage.setItem('compareSelectedOptions', JSON.stringify(updated));
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.contentContainer}>
                {
                    filteredOptions.length > 0 ? (
                                        ...
                                        const element = (
                                            <View key={`${item.id}-${option}`} style={[styles.option, { backgroundColor: color }]}>
                                                <TouchableOpacity style={styles.optionBtn} onPress={() => toggleSelectedCompareOptions(option)}>
                                                    ...
                                                    && <Icon image={require('../assets/icons/selected.png')} width={20} height={20} />
                                                    }
                                                </TouchableOpacity>
                                                <Text style={styles.optionText}>{option}</Text>
                                                <TouchableOpacity
                                                    style={{ position: 'absolute', top: 20, right: 20 }}
                                                    onPress={() => navigation.navigate('RateQuestionScreen', {item: item, option: option})}
                                                >
                                                    <Icon image={require('../assets/icons/edit.png')} width={14} height={14} />
                                                </TouchableOpacity>
                                            </View>
                                        );
                                        globalIndex++;
                        ...
                        <View style={{width: '100%', alignItems: 'center', marginTop: height * 0.2}}>
                                <Image source={require('../assets/nothing.png')} style={{ width: 80, height: 80, resizeMode: 'contain', marginBottom: 26 }} />
            ...
            <TouchableOpacity style={styles.addBtn} onPress={() => setFilterVisible(true)}>
                <Image source={require('../assets/filter-btn.png')} style={{width: '100%', height: '100%', resizeMode: 'contain'}} />
            </TouchableOpacity>
